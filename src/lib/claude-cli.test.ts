...
describe('extractResult', () => {
  ...
  // 후속 회귀(사용자 보고 2026-06-05): claude 가 시작될 때 사용자 환경 MCP 서버(Serena 등)가
  // JSON-RPC 핸드셰이크 `{"jsonrpc":"2.0","result":{...},"id":1}` 를 stdout 으로 먼저 찍으면
  // "첫 균형 잡힌 객체" 가 MCP 메시지가 돼 claude 의 진짜 봉투를 놓쳤다. 이제는 claude envelope
  // 시그너처(result:string 등)에 맞는 객체만 채택해서 MCP 핸드셰이크 뒤의 진짜 봉투를 찾는다.
  it('MCP JSON-RPC 핸드셰이크가 먼저 찍혀도 claude envelope 시그너처로 진짜 봉투 채택', () => {
    const banner = '╔══╗\n║ HEADROOM WRAP: CLAUDE ║\n╚══╝\nProxy ready on http://127.0.0.1:8787\n';
    const serenaJsonRpc =
      '{"jsonrpc":"2.0","id":1,"result":{"protocolVersion":"2024-11-05","capabilities":{"tools":{}}}}';
    const claudeEnvelope = JSON.stringify({ result: 'analyzed', total_cost_usd: 0.012 });
    const stdout = banner + serenaJsonRpc + '\n' + claudeEnvelope;
    // result 가 object 인 JSON-RPC 는 시그너처에서 걸러지고 result:string 인 claude 봉투가 채택됨.
    expect(extractResult(stdout)).toEqual({
      text: 'analyzed',
      usage: { costUsd: 0.012, inputTokens: null, outputTokens: null },
    });
  });

  // 후보 객체 중 어느 것도 claude shape 이 아니면 null (대담한 추측 X).
  it('claude shape 인 객체가 없으면 null — MCP 메시지만 있어도 분석 성공으로 오해 안 함', () => {
    const onlyMcp = '{"jsonrpc":"2.0","id":1,"result":{"protocolVersion":"2024-11-05"}}';
    expect(extractResult(onlyMcp)).toBeNull();
  });
});
...
describe('extractUsageFromStdout — is_error 무관 비용 관측 (리뷰 회귀)', () => {
  ...
  // 후속 회귀(사용자 보고): MCP 핸드셰이크 같은 비-claude 객체가 먼저 와도 사용량 필드(usage/
  // total_cost_usd)가 있는 객체를 찾아 비용 관측이 끊기지 않게.
  it('MCP 메시지가 먼저 와도 비용 필드 있는 객체(claude 봉투)를 찾아 관측', () => {
    const noise = '{"jsonrpc":"2.0","method":"notifications/initialized"}\n';
    const stdout =
      noise +
      JSON.stringify({
        result: 'ok',
