...
export const PRE_REVIEW_OUTPUT_SCHEMA = {
  ...
  properties: {
    ...
    whatToCheck: {
      ...
      items: { type: 'string' },
      description:
        '사용자(메인테이너)가 머지 전/후 즉시 행동할 수 있는 구체적 체크포인트 0-5개. 각 항목 한국어 ' +
        '한 줄(마크다운 금지). **반드시 "어디서(페이지·파일·엔드포인트) → 무엇을(클릭·열기·요청 등 ' +
        '구체적 액션) → 무엇이 보여야 하는지(기대 결과/상태)" 세 요소가 들어간다.** 추상적 표현 ' +
        '("동작 확인", "검토 필요") 금지. 안전한 단순 변경이면 빈 배열로 두는 게 억지로 채우는 것보다 낫다.',
    },
    comments: {
      type: 'array',
...
- whatToCheck — **사용자가 즉시 행동할 수 있는 형태**여야 한다. 형식:
  \`<페이지·파일:line·엔드포인트> 에서 <구체적 액션> → <기대 결과/확인 포인트>\`
  - 좋은 예: \`/inbox 에서 PR #42 행의 머지 버튼이 disabled + "CI 실패" 사유 칩이 보이는지\`
  - 좋은 예: \`src/lib/auth.ts:88 토큰 만료 분기에서 만료 후 재발급 path 가 도는지 (수동 만료 토큰으로 호출)\`
  - 좋은 예: \`/api/webhooks/github POST 로 signature 없는 요청 → 401 반환되는지\`
  - 나쁜 예: \`회원가입 동작 확인\` (어디? 무엇을 클릭? 무엇이 보여야?)
  - 나쁜 예: \`에러 처리 검증\` (어떤 에러를 어떻게 발생? 어디 확인?)
  안전한 단순 변경이면 빈 배열로 두는 게 억지로 채우는 것보다 낫다.
