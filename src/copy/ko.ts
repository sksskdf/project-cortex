export const ko = {
  ...
  roadmap: {
    ...
    // Phase 10.4 — 보드 헤더 요약(별도 '남은 작업' 패널 흡수).
    board: {
      remaining: (n: number) => (n === 0 ? '남은 작업 없음 — 모두 완료' : `남은 작업 ${n}건`),
      hideDone: '완료 숨김',
      showDone: '완료 표시',
    },
    phase: {
  ...
  inbox: {
    ...
    notifications: '알림',
    filter: '필터',
    // 빈 인박스 — 검토 대기 PR 이 없는 정상 상태. 필터/검색 적용 중이면 다른 안내.
    empty: {
      title: '검토할 PR이 없습니다',
      desc: '에이전트가 자신 있는 작업은 자동 머지됩니다. 검토가 필요한 것만 여기 쌓여요.',
      filtered: '조건에 맞는 PR이 없습니다. 필터나 검색어를 바꿔보세요.',
    },
