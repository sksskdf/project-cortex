...
- [~] 위험 플래그·신뢰 점수 정확도 개선 (오탐/미탐 감소, 머지 결과 피드백 학습) — **결정적 부분
      전부 완료**: (1) 피드백 가시화 `getAutoMergeAccuracy`(자동 머지 PR 중 revert 비율) + /reports
      노출. (2) **휴리스틱 정확도 스윕**: auth-domain 미탐(authentication/authorization/authGuard·#258)
      + external-api 오탐(헤더줄·'got' 충돌·#258) + security 자격증명 파일 미탐(apiKey/private-key/
      .pem/keystore) + diff-budget authz 우선순위(#259) 수정. (3) **회귀 평가 세트**(아래 항목)로
      오탐/미탐 고정. **남음(런타임만)**: 머지결과·LLM 출력 corpus 기반 임계·플래그 **자동 튜닝**은
      데이터 축적 + claude CLI 필수 — 샌드박스 불가.
...
- [~] 리뷰 프롬프트/모델 튜닝 + 회귀 평가 세트 (지속적 품질 관리) — **회귀 평가 세트 결정적 부분
      완료**: 정책 게이트(`decideTriage`) 회귀 25건 + **휴리스틱 회귀 평가 corpus 35건**
      (`risk-flags-eval.test.ts` — payment/auth/migration/security/external-api 라벨 케이스 + 과거
      오탐/미탐 고정 사례 author/got/foo..bar/authorization/credential). 패턴 손볼 때 회귀 즉시 검출.
      **남음(런타임만)**: 프롬프트/모델 튜닝의 품질 채점은 실제 LLM 출력을 평가해야 해 claude CLI
      (사용자 머신) 필수 — 샌드박스 불가.
...
- [x] 로드맵 상·하단 중복 뷰 통합 — **보드 단일화(#262 item 1)**: RoadmapOpenItems 남은작업 패널을
      페이지에서 제거하고 RoadmapBoard 가 요약 흡수("남은 작업 N건" 헤더 + "완료 숨김/표시" 토글).
      CRUD 전부 보존. RoadmapOpenItems 컴포넌트 자체는 RoadmapDrawer 가 컴팩트 요약으로 사용 중이라
      유지(삭제 안 함).
...
- [x] Cortex UI 토글 → git roadmap.md — **수동 PR 흐름 완료(#261)**. `serializeRoadmapToMd` +
      `pushRoadmapToGit` + `RoadmapPushButton`. roadmap.md 만 변경(project.yml 자동화 토글 보존).
...
- [x] 자동 양방향 동기화 — **#262 item 3 완료**: roadmapAutoSyncEnabled 프로젝트 토글(migration
      0030, **기본 OFF·opt-in**) + UI 편집 시 fire-and-forget 자동 push + **고정 롤링 브랜치**
      `cortex/roadmap-sync`(편집마다 같은 PR 누적, 스팸 방지). roadmap.md 만 sync(project.yml 자동화
      토글은 안 건드림 — 과거 "pull 때 설정 풀림" 우려 반영). 런타임 검증은 사용자 머신.
...
- [~] `currentUser` 하드코딩 제거 → 인증 주체와 연동 (이슈/PR assignee, 활동 로그).
      **1단계 완료**: `resolveCurrentUser(env)` — CORTEX_USER_NAME/ROLE/INITIALS/GITHUB_LOGIN env 로
      설정 가능(미설정 시 기존 기본값, 무회귀). 테스트 3건. **진짜 인증 주체 연동**(세션에서 동적
      해석)은 인증 게이트(위 항목) 도입 시 후속 — 외부 노출 결정 대기.
...
## 로드맵 상태 — 세션 3 (2026-06-04): 🔴 보안 게이트 + 하드닝 + Phase 10.4 완료

세션 2 의 resolved 이후, §1 의 "자율 가능 범위"를 **코드 품질·보안 하드닝** + **Phase 10.4
양방향 sync 완성**으로 확장. **#231~#262, tests 635 → 734.**

> **운영 원칙(2026-06-04 사용자 지시)**: 작업할 때마다 `.cortex/roadmap.md` 를 항상 갱신한다.
> phase/item 체크박스·상태 섹션 동기화를 작업 PR 과 묶어 처리.
...
- **Phase 10.4 완료**(#261·#262, migration 0030):
  - item 2 수동 PR(#261) — `pushRoadmapToGit` + `RoadmapPushButton`.
  - item 3 자동 sync(#262) — `roadmapAutoSyncEnabled` opt-in 토글 + 고정 롤링 브랜치
    `cortex/roadmap-sync`(편집마다 같은 PR 누적, 스팸 방지) + 마커 루프 차단. roadmap.md 만.
  - item 1 뷰 통합(#262) — `/projects/[id]/roadmap` 보드 단일화: "남은 작업 N건" 헤더 +
    "완료 숨김" 토글 흡수. RoadmapDrawer 는 컴팩트 요약으로 RoadmapOpenItems 계속 사용.
- **Phase 4.7 결정적 부분 완료**: 휴리스틱 정확도 스윕(auth #258 · security 자격증명 파일 ·
  external-api #258 · diff-budget authz #259) + **회귀 평가 세트 35건**(risk-flags-eval.test.ts).
  남은 건 LLM 출력 채점 기반 프롬프트/모델·임계 자동 튜닝뿐 — claude CLI 런타임 필수(샌드박스 불가).
