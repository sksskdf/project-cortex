...
- [ ] **머지 후 워크스페이스 자동 git pull** — 현재 수동(Phase 12 pull 버튼)이라 번거로움. 자동
      머지/직접 머지 후 해당 프로젝트에 연결된 워크스페이스를 자동 git pull(ambient credential,
      best-effort, 실패 무시). 머지 이벤트 → 워크스페이스 매칭(repo) → pull.
