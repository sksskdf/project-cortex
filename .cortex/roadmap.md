...
- [ ] analyzePR 백그라운드 claude CLI spawn (node-pty, 사용자 plan)
- [ ] 리뷰 출력 파싱 → preReview 저장
- [ ] API key 경로는 폴백으로 잔존 (토글)
...
- [ ] check webhook conclusion=failure 감지 → claude CLI spawn 으로 테스트 자동 수정 + push
- [ ] 같은 head_sha 당 반복 한계 + Cortex 관리 PR 만 (사람 PR 은 사람 결정)
- [ ] autoFixTestsEnabled 토글 (디폴트 OFF)
...
- [ ] 에이전트 작업은 worktree 격리 — 메인 작업트리 브랜치 switch 금지 (dev 서버 코드베이스 보호)
