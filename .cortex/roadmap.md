...
- [ ] (A1) **백그라운드 자동화 in-flight 표시** — 충돌해결·테스트수정·리뷰반영이 도는 중임을
      PR 카드/상세에 배지로 (현재 알림으로만 사후 확인). 상태 추적(컬럼/런 기록) 필요.
- [ ] (A2) **토스트 시스템** — 머지·삭제·토글·동기화 결과가 인라인뿐이라 다음 클릭에 사라짐 →
      영속 피드백. (현재 success/error 신호 미약)
- [ ] (A3) **막다른 길 복구 CTA** — 분석 실패 PR 에 '분석 재요청', stuck agent_run 에 '완료 처리'
      노출(현재 발견 불가), disabled 컨트롤에 사유/`준비 중` 라벨.
- [ ] (A4) **error.tsx / loading.tsx / not-found 경계** — 거친 404·빈 화면 깜빡 제거 (현재 전무).
- [ ] (A5) **disabled 컨트롤 설명** — 인박스 알림·필터·정렬 탭, 대시보드 활동·워크로드 '준비 중'
      링크가 무반응 → 툴팁/라벨/`comingSoon`.
- [ ] (A6) 텍스트 상태 중복 제거(AGENTS §2) — PR 상세 '검토 필요' 배지 + 'AI 분석 비활성' 문구 등.
- [ ] (A7) 모달 포커스 트랩/복원, 반응형 — 단일 사용자라 최하 우선순위.
