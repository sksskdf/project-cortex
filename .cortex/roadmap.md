...
- [~] 위험 플래그·신뢰 점수 정확도 개선 (오탐/미탐 감소, 머지 결과 피드백 학습) — **피드백 가시화
      완료**: `getAutoMergeAccuracy` 가 자동 머지된 PR(system·auto-merge·merged) 중 revert 된 비율
      (false-positive)을 측정해 /reports 에 "자동 머지 정확도 N%" 노출. `extractRevertedPrNumber`
      (Revert "… (#N)" → 원본 번호 매칭). 휴리스틱 플래그(`risk-flags`)는 결정적이라 테스트 완비.
      **남음(런타임)**: 이 지표·LLM 출력 corpus 기반 임계·플래그 자동 튜닝은 데이터 축적 + claude
      CLI 필요.
