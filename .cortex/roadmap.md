...
2. [x] **1단계 적용 — R1 `--json-schema` + R2 스킬 헤드리스 전역화**
   ...
   - [ ] 후속: R3 2단계(/reports 집계), R4 권한 정밀화(allowedTools/dontAsk).
...
- [ ] **(결정 필요) 수신 모드 선택 → 구현** — 폴링(A, Cloudflare 제거) vs 고정 터널(B) vs 하이브리드(C).
      A 선택 시 자율 구현 가능: 스케줄러 + 레포별 ETag 캐시 + 설정 토글(웹훅/폴링·주기). sync.ts 재사용.
...
- [x] **라이트 모달 뷰어** — 대시보드 최근 머지에서 행 클릭 시 페이지 이동 없이 모달(`PRPeekModal`)로
      ...
      재사용 가능 — 인박스 적용은 후속(같은 컴포넌트). 렌더 시각 검증은 사용자 머신에서.
      - [ ] 후속: 인박스 목록에도 동일 모달 적용(일관성).
...
- [ ] **(결정 필요) 어느 안부터 + 위치** — 추천 G1(라이브 상태 스트립, 저위험·기존 데이터 종합) →
      G3(quick-capture). 결정되면 G1 자율 구현 가능(집계 lib + 테스트 + 읽기 전용 UI).
- [ ] 상태 한눈 대시보드 강화(G1) — 진행 중 위임/자동화·프로젝트 진척·대기 검토를 한 줄 종합.
- [ ] 세션 요약(G2) — 긴 선형 세션을 단계/결정/산출물로 압축. **stream-json(R6) 선행** 후 진행.
- [ ] **비선형 요청 캡처(G3)** (메타 원칙) — 흩어진 요청 즉시 캡처 → 분류(이슈/로드맵/TODO/폐기).
      이 누락 패턴 자체가 Cortex 가 풀 문제. (현재는 수동 roadmap 박제 — 자동화 검토.)
