...
1. [x] **리서치 보고서** — `docs/CLI-RESEARCH.md` (#204). 공식 문서(headless·cli-reference·subagents)
   기반 기능 매트릭스 + 현재 Cortex 매핑 + ROI 권고(R1~R7). **핵심 발견: 2026-06-15 부터 구독
   플랜의 `claude -p` 가 별도 월간 Agent SDK 크레딧 차감 → "크레딧 0" 전제 변경, 비용 관측 중요↑.**
2. [ ] **1단계 적용** — ROI 최상위: R1 `--json-schema`(파싱 취약점 제거 + Phase 20 "확인할 부분"
   스키마 필드 동시 해결). 이어 R3 비용수집 + R5 `--fallback-model`(작고 06-15 대비).
...
- [ ] **PR 상세 마지막 단락 = "내가 확인하면 될 부분" 요약** — 사전 리뷰 출력에 항상 사용자용
      체크포인트 섹션(무엇을·왜 확인해야 하는지) 포함. analyzePR 프롬프트/파싱 + 상세 렌더.
- [x] **머지 후 워크스페이스 자동 git pull** (#204) — 자동/사람 머지 성공 시 해당 프로젝트의 등록
