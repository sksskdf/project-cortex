...
### 중간 점검 패스 (2026-05-29) — 트랙 C(로드맵 위생, 이 PR) → B(내부 품질) → A(사용성)

C 완료(이 문서). 후속 트랙 B·A 는 별도 PR 시리즈로:

- [ ] **트랙 B · 내부 품질** — (1) 7개 토글 컴포넌트 복붙(~450줄) → 제네릭 ToggleSwitch 기반
      팩토리로 통합. (2) `listProjectsWithStats` N+1(프로젝트당 3 서브쿼리) → GROUP BY 단일 쿼리.
      (3) 핫 컬럼 인덱스(`prs(repoId,status)`·`projects(installationId)`). (4) AI 분석 실패 ≠ 머지
      실패 알림 종류 분리(현재 'auto-merge-failed' 재사용). (5) 토글 동시 클릭 race 가드.
- [ ] **트랙 A · 사용성 스윕** — 4대 화면(대시보드·인박스·PR상세·/projects) 일관성·빈 상태·로딩
      상태 점검 + 텍스트 최소화 원칙 준수 + 반응형(위 항목과 통합).
