...
| 2026-05-21 | **done 카테고리 stripe 일관성 검증** | PR #81 (머지/닫힘 PR 은 위험 stripe 표시 안 함) 머지 후 검증. 사용자 보고: #78 confidence 0 이 노란 stripe + #61 migration 플래그가 빨간 stripe — 머지된 PR 들에 일관성 없음. 수정 후 모든 done PR 이 회색 (stripe 없음) 으로 일관 표시. 이 PR 도 자동 머지되면 done 카테고리에서 stripe 없이 보여야 정상 |
