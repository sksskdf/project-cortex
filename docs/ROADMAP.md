...
| 2026-05-21 | **PR 상세 CI 가드 + 차트 색상 토큰 정정** | (1) PR 상세의 `mergeBlockedByCI` 가 `testsPassed === null` (대기) 만 막고 `testsPassed === false` (실패) 는 안 막아서 CI 실패 PR 을 PR 상세에서 머지 가능했음 (사용자 시그널: "테스트 실패 시 대시보드 머지 버튼은 막혀있는데 PR 상세로 이동하면 머지 가능함"). 인박스 행 `deriveRowActions` 와 일관성 맞춰 `testsPassed !== true` 로 정정. `mergeBlock.ciFailed` 카피 추가하여 PR 상세 메시지가 대기/실패를 구분. (2) ReportsCharts 의 막대 색이 검정으로 렌더되던 버그 — `--ds-color-state-info-accent` · `--ds-color-state-warning-accent` 가 dark.css 에 정의 안 됐는데 SVG fill 속성에 var() 가 invalid 일 때 검정 fallback. 실재 토큰 (`secondary-01`, `state-warning-base`, `primary-05`) 으로 교체. `NotificationDropdown` 의 같은 토큰 사용처도 같이 정리 |
