...
| 2026-05-29 | **중간 점검 — 코드베이스·로드맵 검토 + 최적화 패스 박제 (트랙 C→B→A)** | 핵심 자동화(리뷰·triage·자동머지·충돌/테스트/리뷰 자동수정) 완성 + 멀티 프로젝트 안전(#179~#187) 후, Phase 16~19 신규 표면 진입 전 **계획적 정리 패스** 결정. 신호: 실사용 중 러프엣지가 줄줄이 발견됨(뮤트 불완전·목록 머지버튼 미disable·CI 없는 레포 무한대기·토글 난잡·PR번호 누락·사람PR 자동화 제외) → 임기응변보다 체계적 스윕이 효율적. 빅뱅 리팩터 금지, **테마별 작은 PR 시리즈**. **트랙 C(로드맵 위생, 이 PR)**: stale 서술 정정(13.1/13.2/13.3 'agent PR 만'→작성자 무관) + 미반영 done 항목 반영(#179·#182~#187) + 클러스터링 유지·판단 보류 결정. **트랙 B(내부 품질)**: 토글 7개 복붙→제네릭 팩토리, `listProjectsWithStats` N+1→단일 GROUP BY, 핫 컬럼 인덱스, AI 분석실패≠머지실패 알림 분리, 토글 race 가드. **트랙 A(사용성)**: 4대 화면 일관성·빈/로딩 상태·반응형(Phase 15). 순서 C→B→A (B 가 A 의 UI 작업 토대) |
