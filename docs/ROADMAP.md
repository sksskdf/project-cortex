...
**DoD**
- 위험 플래그·CI 실패/대기 없고 사람 작성 아닌 에이전트 PR이 자동 머지됨 (신뢰 점수 임계값 게이트는 2026-05 제거 — Decision Log)
...
| 2026-05-22 | **`Closes #PHASE-<key>` cascade — phase 매칭 시 item 들도 `doneByPrId` 자동 연결** | 사용자 시그널: "가장 최근 PR 과 로드맵 확인해보면 로드맵 ↔ PR 연결이 누락". 진단 결과 코드는 모두 살아있음 (PR #101 복원 성공) — 진짜 문제는 (a) 최근 PR body 에 마커 무, (b) 기존 `Closes #PHASE-<key>` 가 `roadmapPhases.status` 만 바꾸고 item 의 `doneByPrId` 는 안 채워서 UI 의 `#N` 링크가 안 보임. **수정**: `matchAndApplyDoneFromPR` 에 cascade 추가 — phase 매칭 시 그 phase 의 `doneByPrId IS NULL` + `status != 'done'` item 들을 이 PR 로 자동 연결. 명시 `Closes #ITEM-N` 으로 다른 PR 에 연결된 건 가드. 이 변경으로 사용자가 PR body 에 `Closes #PHASE-11` 한 줄만 적으면 phase 11 의 모든 미완료 item 옆에 `#N` 링크 자동 표시. PR #102 본문에 `Closes #PHASE-11` · `Closes #PHASE-12` 추가 + `.cortex/roadmap.md` 10.2/11/12 항목 stale 상태 갱신 |
| 2026-05-22 | **PR 생성 시 자동 cascade 발화 — 메타데이터 3중 안전망** | 사용자 시그널: "자동 연결 규칙을 PR 생성 시 활용하여 자동으로 처리될 수 있게끔 메타데이터 작업". cascade 규칙은 살아있어도 PR body 에 `Closes #PHASE-<key>` 가 빠지면 발화 X. **정적 메타데이터로 누락 방지** — (1) `.github/PULL_REQUEST_TEMPLATE.md` 최상단에 `Closes #PHASE-` 자리 + 주석 가이드 (GitHub 웹 UI · `gh pr create` 양쪽 자동 prefill) + 체크리스트 항목. (2) `AGENTS.md §8` 강한 룰 추가 — "PR 본문 최상단에 `Closes #PHASE-<key>` 필수" + 예시 + 브랜치명 `feat/phase<key>-<name>` 컨벤션. (3) `docs/CONVENTIONS.md §10·§13` 동일 룰 미러. AI 에이전트 (Claude Code) 와 사람 모두 같은 단일 출처 (AGENTS.md) 로 빠뜨릴 가능성 최소. **후속 가능 옵션** (이번 PR 비-포함): `pull_request.opened` webhook 에서 브랜치명 → phase key 추출 → PR body 미포함 시 Octokit PATCH 자동 추가 (invasive 라 별도 박제 + 사용자 OK 후) |
| 2026-05-22 | **Phase 11 notes (메모) + 브라우저 알림 토글 진짜 스위치로 교체** | 사용자 시그널 (두 건 묶음): (a) "토글로 바꿔달랬더니 이게 뭐임? 토글 모름? 두번째 첨부한게 토글이잖아" — chip 형태 (border + 텍스트 ON/OFF) 를 진짜 슬라이딩 스위치 (track 36×20 + knob translateX) 로 교체. 라벨 '브라우저 알림' 은 스위치 좌측 별도. (b) "다음 작업 진행" — Phase 11 의 남은 notes 구현 (todos 와 같은 패턴, 자연 후속). `notes` 테이블 (title + body markdown + pinned + projectId/prId optional FK) + `lib/notes.ts` CRUD + 검색 (LIKE) + `previewWithMatch` 매칭 위치 highlight + `actions/notes.ts` + `/notes` 페이지 + `NotesView` (검색 toolbar + add form + inline expand · 편집 · 핀 · 삭제 confirm). 사이드바 NavItem `메모` + 카운트. 마이그레이션 0013. 9 unit tests. **빌드 메모**: NotesView (client) 가 lib/notes (db) 직접 임포트 시 node:fs 끌어 webpack 실패 → `lib/notes-preview.ts` 로 순수 헬퍼 분리, 타입은 `import type` (TS 컴파일 시 elide) |
| 2026-05-22 | **Phase 8 수동 레포 등록 마법사 + Phase 12 IDE handler 병렬 묶음** | 사용자 선택 "둘 다 UI · 영역 독립 · 짧음 — 조각 채우기". **Phase 8**: `/projects` 페이지 상단 "+ 프로젝트 추가" 버튼 + inline form. `addProjectManually(slug, name?)` lib 추가 — slug 형식 검증 (`^[A-Za-z0-9._-]+/[A-Za-z0-9._-]+$`) + 중복 거부 + insert with `installationId=null` + `autoMergeEnabled=false`. 후속 webhook 도착 시 `handlePullRequestWebhook` 의 기존 자동 onboard 로직이 같은 slug 매칭으로 `installationId` 채움 (코드 변경 X — 자연 합류). 시드 섹션 라벨 정리 ('시드/데모' → 'App 미설치 (대기/시드)'). **Phase 12**: WorkspaceCard 의 RegisteredView 에 `vscode://file/<path>` + `cursor://file/<path>` 두 anchor 추가. `encodeURI` 로 한글/공백 safe. OS 핸들러 부재 시 브라우저가 무시 (안전). 4개 새 테스트 (addProjectManually). 318→322 통과 |
| 2026-05-22 | **squash 누락 3회 재발 — 검증 룰 박제 + 토글 스위치 재복원** | PR #95·#100·#103 에서 동일 사고: PR 생성 후 같은 브랜치로 추가 commit push → GitHub squash 가 머지 시점 head SHA 까지만 합치고 이후 commit 누락. PR #103 의 브라우저 알림 토글 commit (chip→슬라이딩 스위치) 이 머지에 안 잡혀 master 에 chip 형태로 남음 (사용자: "알림 토글은 계속 그대로인 것 같은데 도대체 언제 수정할거임?"). **수정**: 누락 commit `c1b60fc` 를 새 브랜치로 cherry-pick 재적용 (PR #106). **박제**: `AGENTS.md §8` 에 "PR 생성 후 추가 push 했으면 머지 직전 head SHA == 마지막 push SHA 확인 + 머지 후 `git show origin/master:<파일>` 로 반영 확인" 강한 룰 추가. CI 는 이 사고를 못 잡음 — 테스트 실패가 아니라 commit 자체가 머지에서 빠지는 것이라 (CI 게이트는 이미 `.github/workflows/ci.yml` 존재) |
| 2026-05-24 | **squash 누락 4회차 — 박제 자체가 누락 + 운영 룰 전환** | PR #106 에서 토글 commit 으로 PR 생성 후 squash-검증 박제 commit (`e2b7fd3`) 을 추가 push → 또 머지에서 누락 (박제 룰 자체가 squash 에 빠지는 아이러니). **근본 해결 — 운영 룰 전환**: "PR 생성 후 push" 자체를 금지. 브랜치에 모든 commit 을 쌓아 push 를 끝낸 **뒤에야** PR 생성 (cortex-harry 자동 머지가 빠르게 돌아 race 발생). 이 PR 부터 적용 — 박제 재적용 + 대시보드 핀 메모 위젯 + Phase 7 CI 완료 마킹 모두 push 완료 후 PR 생성. **Phase 11 마무리**: `listPinnedNotes(limit)` + `DashboardNotesWidget` (핀 고정 메모 최대 5개 → 빠른 참조 보드). Phase 7 배포 자동화 = 이미 존재하는 `.github/workflows/ci.yml` 로 완료 마킹. 322→324 테스트 |
| 2026-05-26 | **Phase 13 — Claude CLI 터미널 임베드 + 전역 드로어** | `/agents` 에 xterm.js 터미널(#108): App Router 가 WebSocket upgrade 불가라 **커스텀 Next 서버**(`server.ts`, tsx)로 Next+`ws` 한 프로세스, `node-pty` 가 `claude` CLI 만 화이트리스트 spawn (cwd=등록 워크스페이스). 새 dep: `@xterm/xterm`·`@xterm/addon-fit`·`node-pty`·`ws` (런타임 변경 — `npm dev`/`start` 가 커스텀 서버). 사용성 개선(#116): 터미널을 페이지에서 떼어 **AppShell 마운트 전역 드로어**(sticky 런처+전체화면)로 → 화면 이동에도 세션 유지. `/agents` 라우트 은퇴(→홈 redirect), 사이드바 '에이전트'=드로어 토글, 대시보드 '에이전트 시작'=드로어 진입(#110). Windows 는 `claude.cmd` 라 `cmd.exe /c` 로 spawn(#111). **2단계(새로고침에도 세션 유지=서버 detached 세션+scrollback)는 후속** |
| 2026-05-26 | **동기화·자동머지 운영 사고 3건 fix** | (a) `reconcileProject` 가 PR 실제 state/merged 를 무시(`action='opened'`+`merged=false` 하드코딩) → 이미 머지/닫힌 PR 까지 `review-needed` 로 되살려 인박스 오염(GitHub 0, Cortex 94) + 가짜 '자동머지' 알림. 실제 state 반영으로 수정(#109). (b) `attemptAutoMerge` race — 동시 webhook 으로 한 호출 머지 성공·다른 호출 'already merged' 오류가 triage 를 `human-review` 로 덮어써 대시보드 '수동' 오분류 + 모순 알림 → **in-flight lock**(#111). (c) `listOpenPullRequests` 가 첫 페이지(100)만 → **페이지네이션** 추가, 100+ PR 레포 누락 해결(#113). 박제: reconcile 은 단방향 add 가 아니라 GitHub 실제 상태 반영 |
| 2026-05-26 | **트라이아지 정책 — 위험 아니면 자동 머지** | 사용자 시그널: "위험 아니면 다 자동 머지". `decideTriage` 의 **신뢰 점수 임계값(`AUTO_MERGE_THRESHOLD`) 게이트 제거**(#117). 차단은 위험 플래그·CI 실패/대기·사람 작성·autoMerge off 만 유지. 신뢰 점수는 표시용으로만. Phase 5 DoD '90+' 문구 갱신, `.cortex/roadmap.md` Phase 5 'runTriage 5 조건' 갱신 |
| 2026-05-26 | **TODO·메모 프로젝트별 + 개인 / 텍스트 최소화 원칙** | (a) `todos`·`notes` 추가 폼에 프로젝트 선택 + 목록 프로젝트 필터(전체/개인/프로젝트별), `projectId=null`=개인(#114·#115). 스키마는 이미 `projectId` FK 보유 — UI 만 연결. (b) PR 상세 테스트 상태에 'CI **측정중**'(check run in_progress 라이브 조회) 추가(#112). (c) **텍스트 최소화** 를 `AGENTS.md §2` 디자인 핵심 원칙으로 박제(#118) — UI 가 이미 보여주는 상태(토글 ON/OFF 등)를 글자로 중복하지 않음. 설정 '활성'·결과 메시지 제거, 워크스페이스 IDE 링크 제거(#109)도 같은 맥락 |
| 2026-05-26 | **프로젝트 등록 유연화 계획 (PAT) — Phase 8 후속** | 현재 등록은 webhook(첫 PR) 자동 onboard 또는 수동 slug 입력뿐. 3 흐름 추가 결정: (a) **설치된 GitHub 리포 선택 등록** — app-level JWT(`getOctokitForApp`)+`apps.listInstallations`/`listReposAccessibleToInstallation`, `installationId` 즉시 세팅(추가 secret 0). (b) **새 리포 생성** — App 설치 토큰은 개인 리포 생성 불가라 fine-grained **PAT** 를 설정에 저장 → `repos.createForAuthenticatedUser`. (c) **로컬 경로 먼저 등록 → 리포 연결** — `projects.slug` nullable/draft 스키마 필요. 슬라이스 (a)→(c)→(b) 순 별도 PR. 생성/선택 리포에 webhook 오려면 App 이 그 리포에 설치돼 있어야 함('all repos' 설치 권장) |
| 2026-05-26 | **에이전트 세션 새로고침 유지 + 트라이아지 신뢰점수 명확화** | (a) 새로고침 시 claude 세션 유실 → 서버 **detached 세션**(#120): pty 를 sessionId 로 보관, ws close 시 안 죽이고 재접속 시 scrollback replay, 구독자 없이 10분 reap, 명시적 kill. 클라이언트는 sessionId 를 localStorage 저장 → 새로고침 자동 resume. 터미널 폰트 monospace 교정(DS 에 코드용 mono 토큰 없음 — number=sans, digital=7세그). (b) **'위험' ≠ 신뢰 점수**: 자동머지 차단의 '위험'은 위험 플래그(결제·인증·마이그레이션·보안·신규외부API)지 점수가 아님. confidenceTier 경계는 90/70/50 (high/medium/low/critical). #117 이 90점 게이트를 제거해 78점(medium)도 자동머지됨 — 점수 하한 복원 여부/값은 사용자 결정 대기 |
| 2026-05-26 | **로드맵·온보딩 보강 + Phase 15(UI/UX·최적화) 신설** | 사용자 시그널: "메모리 없는 새 세션도 바로 이어가게 + 로드맵에 UI/UX·최적화 내용이 없음". (a) Decision Log 를 #120 + 트라이아지 명확화까지 최신화 — 새 세션은 README→AGENTS→이 일지 tail + `.cortex/roadmap.md` 의 `[ ]` 항목으로 현재 상태·다음 작업을 파악. (b) **Phase 15 — UI/UX·최적화(지속)** 신설: 텍스트 최소화·DS 준수·죽은 코드 제거 원칙 + 잔여(ReportsCharts 색 토큰화, 패키징 전 전체 검토). (c) Phase 13 에 드로어 드래그 리사이즈(미구현) 항목 추가. (d) `AGENTS.md §0` 에 '현재 상태/다음 작업' 포인터 추가 |
| 2026-05-26 | **다음 작업 4건 박제 — 로드맵 통합·세션 연속성·세션 관리·AI 리뷰 크레딧 제거** | 끊긴 병렬 worktree 6건(Phase 7 backup/pino/sentry · Phase 9 서비스 · Phase 13 새 이슈 · Phase 14 도움말) 마무리(#122~#127) 후 사용자 지시 4 방향. 진행 방식 = **4건 모두 박제 먼저 → 우선순위 순 별도 PR**. 우선순위(작은 것 먼저): **(1) Phase 10.4 로드맵 양방향 sync + 중복 뷰 통합** — `/projects/[id]/roadmap` 상·하단 중복 뷰를 상단 베이스로 합치고 거기에 '산출물 추가'(item 생성/편집) 기능 통합. 이후 UI 토글 → git `roadmap.md` 자동 PR(cortex marker 로 무한 sync 방지). **(2) Phase 16 세션 연속성(.cortex work-state)** — 이번 세션이 겪은 '끊긴 병렬 작업 파악 비용'을 줄이려 `.cortex/`에 진행 중 브랜치·worktree·다음 단계를 적은 work-state 파일을 두고 새 세션이 README→AGENTS 다음으로 먼저 읽음(메타 schema v1, 사람·에이전트 공용). **(3) Phase 13 세션 관리** — #120 detached 세션 위에 다중 세션 목록·종료·이름·전환 UI(현재는 단일 sessionId resume 만). **(4) Phase 4.6 터미널 에이전트 기반 리뷰(크레딧 0)** — `analyzePR` 의 Anthropic API key 호출(Phase 4/4.5, 크레딧 소모)을 Phase 13 터미널 인프라(`node-pty` `claude` CLI, 사용자 plan)로 백그라운드 spawn 해 리뷰 결과 파싱 → `preReview` 저장. Decision #486 'CLI spawn = 크레딧 0' 확장 — 가장 큰 아키텍처 변경이라 설계 박제 후 착수, API key 경로는 폴백으로 잔존 |
| 2026-05-26 | **운영 자동화 4건 + DB 중앙화 박제** | 병렬 PR 마무리(#123 pino 병합충돌 해소 — Sentry `captureError` 와 pino `logger.error` 양립 / #126 prettier 실패 수정) 중 사용자가 관찰한 운영 마찰을 후속 작업으로 박제. **(1) Phase 13.3 — CI/테스트 실패 자동 수정**: check webhook `conclusion=failure` 감지 시 claude CLI spawn 으로 테스트 자동 수정 + push (13.1/13.2 와 동일 가드: `head_sha` 당 반복 한계, Cortex 관리 PR 만, `autoFixTestsEnabled` 디폴트 OFF). **(2) Phase 5 — 사람 작성 PR 자동 머지 옵트인**: 현재 자동 머지는 '사람 작성 아닌 에이전트 PR' 만(line 249 설계) — 사용자 본인이 연 PR(#123/#126)이 사람 작성으로 인식돼 자동 머지 안 됨. 라벨/PR 본문 마커로 'Cortex 관리' 표시 시 자동 머지 대상에 포함하는 옵트인 추가. **(3) Phase 16 — 에이전트 worktree 격리**: 메인 작업트리에서 브랜치 switch 시 사용자 dev 서버가 보는 코드베이스가 바뀌는 마찰 → 에이전트 작업은 worktree 로 격리(메인트리 switch 금지). work-state 와 함께. **(4) Phase 17 — DB 중앙화**: 로컬 SQLite 가 머신/환경마다 분리돼 작업 상태 일관성이 깨짐 → 중앙 DB 로 멀티 환경 일관성 확보(단일 사용자 가정 유지, 환경별 접속 설정 + 마이그레이션 경로, 오프라인 폴백 검토). 진행 방식 = 박제 먼저 → 우선순위 순 별도 PR |
| 2026-05-27 | **Phase 4.6 구현 — 사전 리뷰 claude CLI 백엔드(크레딧 0)** | #516(4)/Decision #486 확장 구현. `lib/claude-cli.ts` 신규 — 헤드리스 `claude -p --output-format json` spawn (대용량 diff 는 stdin, `resolveClaude` 화이트리스트, Windows `claude.cmd`→`cmd.exe /c`, DI 훅 `setClaudeRunner`). `pre-review.ts` 의 Haiku triage·Opus 본분석 LLM 호출을 `env.preReviewBackend()` 로 분기 — **디폴트 `cli`(사용자 Claude plan, Anthropic API 크레딧 0)**, `CORTEX_PRE_REVIEW_BACKEND=api` 면 기존 SDK 경로 폴백. CLI 는 JSON schema 강제 불가라 프롬프트 지시 + `parseJsonFromText`(코드펜스 제거) + zod 검증. **자동 머지 미발화와 연결**: AI 를 크레딧 아끼려 끄면 preReview 없음→triage `no-pre-review`→`review-needed` 고착이라 자동 머지 비발화 — 크레딧 0 이면 AI 를 켠 채 둘 수 있어 흐름 회복. ⚠️ claude CLI 부재 환경에선 typecheck 만 통과 — 실기기/CI 런타임 검증 필요 |
| 2026-05-27 | **Phase 13.2 구현 — 병합 충돌 자동 해결 (rebase→merge 전략 변경)** | #494 구현. 마이그레이션 0014 + `projects.autoResolveConflictsEnabled`(디폴트 OFF). `conflict-resolve.ts` — auto-merge 가 `mergeable_state='dirty'` 감지 시(토글 ON) git 조작은 Cortex 가 결정적으로 수행하고 충돌 파일 마커 해소만 claude CLI(`--dangerously-skip-permissions`, cwd=등록 워크스페이스)에 위임. **로드맵의 rebase+force-push 에서 merge(base→head)로 변경** — 프로젝트가 squash 머지라 머지 커밋은 어차피 소멸 + **force-push 불필요 → blast radius 최소**(원격 덮어쓰기 위험 0). 가드: 토글 OFF·agent PR 만·등록 워크스페이스·fork 제외·충돌 파일 10개 한계·잔존 마커 검사·실패 시 `merge --abort`+PR 코멘트+알림. push 가 synchronize webhook 발사→재트라이아지→재머지. `.cortex/project.yml` `automation.auto_resolve_conflicts` 동기화. agent_runs 기록은 `issueId` notNull 제약으로 보류(logger+알림 대체). 충돌 토글 UI(/settings)는 후속. ⚠️ 런타임 검증 필요 |
| 2026-05-27 | **Anthropic API 전면 제거(#136) + 충돌해결·테스트수정 재구현(#135) + 에이전트 UI 정리(#131)** | (a) #136 — Phase 4.6 의 '폴백 잔존' 계획을 뒤집어 **`@anthropic-ai/sdk` 의존성·`anthropic.ts`·`ANTHROPIC_API_KEY` 경로 전면 제거**. 모든 AI 작업(사전 리뷰·triage)이 claude CLI(사용자 plan, **API 크레딧 0**), 폴백 없음. (b) #135 — Phase 13.2(병합 충돌 자동 해결)는 스택 PR #133(base=#132)이 닫히고 **#135 로 재구현 머지** + **Phase 13.3(CI/테스트 실패 자동 수정)** 동반(autoFixTestsEnabled 디폴트 OFF, head_sha 반복 한계, Cortex 관리 PR 만). (c) #131 — 에이전트 별도 탭·대시보드 '에이전트 시작' 버튼·터미널 hint 제거 → 진입은 전역 드로어로 단일화 |
| 2026-05-27 | **로드맵 정합성 재동기화 + 이슈 목록 뷰 갭 박제** | 다른 세션의 #122~#136 머지로 `.cortex/roadmap.md` 의 다수 `[ ]` 가 실제 완료와 어긋나 일괄 체크(Phase 4.6·7(pino/sentry/backup)·9·13·13.2·13.3·14) + 틀린 서술 정정(Phase 4.6 '폴백 잔존'→전면 제거, Phase 13 '에이전트 시작 헤더 액션'→#131 로 제거됨). **미구현 갭 기록**: '새 이슈'(#126 `NewIssueDialog`)는 됐지만 **이슈 목록 뷰**(이슈별 claude 세션 진행상태 + 이슈↔결과 PR 연결)는 미구현 → Phase 13 항목 추가. '위임 토글 제거→항상 위임' 전환은 동작 변경이라 별도. 드로어 '드래그 크기 조절(폭/높이)'도 미구현(현재 전체화면 토글+도킹만 #129) |
| 2026-05-27 | **세션 연속성·워크스페이스 격리 전략 (Claude 앱 대비 UX 격차)** | 사용자 관찰: Claude 앱(클라우드)은 (a) 세션이 쭉 이어지고 (b) 깃헙 리포의 격리된 클론에서 작업해 로컬이 안 더러워지는데, Cortex 터미널은 (a) 서버 종료 시 pty(node 자식 프로세스)가 죽어 세션 유실 (#120 detached 는 새로고침·이동만 견딤, 서버 재시작은 못 견딤) (b) 로컬 워크스페이스에서 직접 작업해 dev 서버 코드베이스·브랜치가 바뀜. **해법 박제(추천 경로 = 1+2a)**: (1) **git worktree 격리** — 세션마다 메인 작업트리가 아닌 별도 worktree(임시 디렉토리 + 전용 브랜치)에서 claude spawn → 메인 체크아웃 무오염(Phase 16). (2a) **`claude --resume <세션id>`** — 대화는 claude 가 `~/.claude` 에 영속하므로 pty/서버가 죽어도 세션 id 저장 후 resume 으로 대화 연속(서버 재시작 극복, tmux 데몬 불필요). (2b) tmux 분리 데몬 — 라이브 프로세스(실행 중 명령·출력)까지 보존하나 무거워 후순위. 추가 전략: 로컬 spawn 대신 **클라우드 Claude Code(웹/Action) 트리거** — 통합 비용 큰 별도 전략 결정 |
| 2026-05-29 | **멀티 프로젝트 안전·자동화 토글 묶음 (#179~#187)** | 조직 App 연결 후 인박스 오염·자동화 제어 부재를 해소한 연속 작업. **(#179)** 프로젝트별 AI 사전 리뷰 토글(전역 토글과 AND). **(#180/#183/#185)** 프로젝트 뮤트 — 신규 감지 레포 기본 뮤트(migration 0021), 인박스 카운트·대시보드 위젯까지 완전 적용, 뮤트 시 하위 자동화 토글 비활성. **(#181/#182/#186)** /projects 카드 자동화 UI — 충돌해결 토글 UI + 6개 토글 스위치화('자동화' 그룹 + 마스터 'Cortex 관리' 스위치) + 테스트수정·리뷰반영 토글로 패널 완성. **(#184)** `lib/merge-gate.ts` 단일 게이트 — 목록 머지버튼 disable+사유(상세와 공유) + **CI 없는 레포 자동 머지**(testsPassed null + mergeable_state='clean' 이면 통과, prs.mergeableState 저장 migration 0022). **(#187)** 세 claude 자동화(충돌해결·테스트수정·리뷰반영)의 `authorKind !== 'agent'` 가드 제거 — **단일 사용자 가정상 사람 PR 도 내 PR**(fork 가드·토글 OFF 디폴트가 안전망), 충돌 해결은 자동 머지 안 타는 PR 도 sync 가 dirty 감지 시 발화. 최근 머지 목록 #PR번호 노출 |
| 2026-05-29 | **중간 점검 — 코드베이스·로드맵 검토 + 최적화 패스 박제 (트랙 C→B→A)** | 핵심 자동화(리뷰·triage·자동머지·충돌/테스트/리뷰 자동수정) 완성 + 멀티 프로젝트 안전(#179~#187) 후, Phase 16~19 신규 표면 진입 전 **계획적 정리 패스** 결정. 신호: 실사용 중 러프엣지가 줄줄이 발견됨(뮤트 불완전·목록 머지버튼 미disable·CI 없는 레포 무한대기·토글 난잡·PR번호 누락·사람PR 자동화 제외) → 임기응변보다 체계적 스윕이 효율적. 빅뱅 리팩터 금지, **테마별 작은 PR 시리즈**. **트랙 C(로드맵 위생, #188)**: stale 서술 정정(13.1/13.2/13.3 'agent PR 만'→작성자 무관) + 미반영 done 항목 반영(#179·#182~#187) + 클러스터링 유지·판단 보류 결정. **트랙 B(내부 품질)**: 토글 7개 복붙→제네릭 팩토리(#189), N+1→배치쿼리+인덱스(#190), 자동화 알림 종류 분리. **트랙 A(사용성)**: 4대 화면 일관성·빈/로딩 상태·반응형(Phase 15). 순서 C→B→A (B 가 A 의 UI 작업 토대) |
| 2026-05-29 | **전체 흐름·사용성 홀 분석 + 자동화 가시성 1단계** | 사용자 요청으로 UI/UX·기능 전체 홀 분석(병렬 정찰 2 + 세션 지식 종합). **핵심 결론**: 이 도구의 본질은 자동화 신뢰인데 **자동화가 조용히 돌고 조용히 실패**하는 게 최대 홀. 단일 사용자 localhost 라 반응형·a11y 보다 **피드백·가시성·막다른 길 복구** 우선. 발견 홀 → Phase 15 트랙 A(A1~A7) 박제. **1단계 구현(이 항목)**: 자동화 알림 종류 분리 — `conflict-resolve`/`test-fix`/`review-fix`/`analyzePR` 실패가 모두 `auto-merge-failed` 재사용이라 "무엇이 실패했는지" 구분 불가하던 걸 전용 kind(`analysis-failed`·`conflict-resolve-failed`·`test-fix-failed`·`review-fix-failed`)로 분리. + **성공도 알림**(`conflict-resolved`·`tests-fixed`·`review-addressed`) — 이전엔 자동화가 돌아도 사용자가 인지 못 했음. `auto-merge.ts` 의 진짜 머지 실패만 `auto-merge-failed` 유지. NotificationDropdown dot 색 + kindLabel copy 확장. (알림 kind 는 drizzle text enum — 마이그레이션 불필요) |
...
**진척 (2026-05-22)** — 1차 + 후속 모두 출시. `todos` 테이블 + `/todos` 페이지 + 대시보드 위젯 + 사이드바 카운트 + Server Actions + 9 tests. **`notes` 테이블 + `/notes` 페이지 + NotesView (검색 toolbar + add form + inline expand 편집 + 핀 토글 + 삭제 confirm) + 사이드바 NavItem + 9 tests** (Phase 11 메모 후속, 마이그레이션 0013). `events` 캘린더는 후속 (사용 데이터 본 뒤).
...
> 사용자 시그널 (반복): "불필요한 거 걷어내고 고도화 + 최적화", "디자인 시스템 안 따르거나 어색한 부분 신경써서 최적화". 특정 시점 phase 가 아니라 **지속 작업** — 새 화면/기능마다 적용.

**핵심 원칙**

- **텍스트 최소화** (`AGENTS.md §2` 박제) — UI 가 이미 보여주는 상태를 글자로 중복하지 않음 (토글 ON/OFF 옆 '활성' 라벨·"~했습니다" 결과 메시지 등 제거).
- **디자인 시스템 준수** — 색·간격·radius 는 토큰만, 한국어 카피는 `ko.ts`. 인라인 hex·인라인 카피는 회귀로 간주.
- **죽은 코드 제거** — 미사용 export·영구 disabled UI·중복 헬퍼는 발견 즉시 정리.

**진행 / 잔여**

- [x] 텍스트 최소화 원칙 박제 (#118) · 죽은 코드 정리 (#113) · 터미널 폰트 monospace (#120)
- [ ] `ReportsCharts` 하드코딩 색 → 디자인 토큰 (Recharts 가 CSS var 직접 못 읽어 런타임 read 방식 필요)
- [ ] 패키징 전 전체 UI/UX 검토 패스 — 코드 레벨 감사(인라인 카피·DS 위반·dead code) + 사용자 스크린샷 루프

**검증 메모** — 원격(web) 샌드박스에선 헤드리스 브라우저 구동 불가 (Chromium 다운로드 차단, apt chromium 은 snap 스텁). UI 시각 검증은 **사용자 스크린샷 루프** 또는 로컬 실행. 코드 레벨(토큰·카피·dead code)은 grep/감사로 확인.
