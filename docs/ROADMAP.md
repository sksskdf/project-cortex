...
#### Phase 10.1 — 메타 + 로드맵 단방향 git → Cortex 동기화 (구현 1순위)
...
#### Phase 10.2 — push webhook 자동 sync (구현 2순위)

- `push` webhook 받을 때 `.cortex/` 디렉토리 변경 감지 → 자동 reconcile
- 변경 path 가 `.cortex/` 로 시작하는 경우만 sync 호출 (다른 push 는 무시)
- default branch push 만 (feature 브랜치 무시)
- AI 분석 트리거하지 않음 (메타 변경은 PR 분석과 별개)
